...
Iteracja 4 ładuje sprawy wraz z ich rolami dłużników — trzy tabele stagingowe (`dbo.sprawa`, `dbo.sprawa_rola` oraz ponownie polimorficzna `dbo.atrybut` z filtrem `att_atd_id = 4`) zasilają sześć tabel produkcyjnych (`rachunek_bankowy`, `sprawa`, `operator`, `sprawa_rola`, `atrybut_wartosc`, `atrybut_sprawa`). Wszystkie przejścia są klasy **C**, zależne od słowników z iteracji 1 (`sprawa_typ`, `sprawa_rola_typ`, `atrybut_typ`) oraz od `mapowanie.dodani_dluznicy` zbudowanego w iteracji 2. Iteracja jest warunkiem koniecznym dla iteracji 5 (akcje i rezultaty) oraz iteracje 6-7 (wierzytelności i dokumenty).

Iteracja 4 jest iteracją o największym rozgałęzieniu — jeden wiersz staging `sprawa` może wygenerować do trzech wierszy w prod: distinct rekord w `rachunek_bankowy` (po `sp_numer_rachunku`, idempotencja po `rb_nr` bez `ext_id`), główny rekord w `sprawa` (IDENTITY prod PK, staging `sp_id → sp_ext_id`, range-based idempotencja) oraz warunkowy rekord w `operator` gdy `sp_pracownik IS NOT NULL` (FK `op_us_id` rozwiązywany przez prod `GE_USER.US_LOGIN` — tabela systemowa poza schematem migracji). Mapowanie staging→prod PK zapisywane jest do `mapowanie.dodane_sprawy` — źródła FK dla iteracji 5/6/7. `sprawa_rola` używa MERGE po composite key (`spr_sp_id`, `spr_dl_id`), a atrybuty spraw współdzielą procedurę `usp_migrate_atrybut_wartosc` z iteracji 2 (zmiana tylko parametru `@att_atd_id = 4` i docelowej junction na `atrybut_sprawa`). Szczegóły per prod-tabela w sekcjach `### dbo.<tabela>`; walidacje referencyjne, formatu i biznesowe w sekcji [Powiązania](#powiazania) poniżej.

<div class="iter-meta">
  <span>Iteracja: 4</span>
  <span>Zależności: Iteracja 1 (sprawa_typ, sprawa_rola_typ, atrybut_typ) + Iteracja 2 (mapowanie.dodani_dluznicy)</span>
</div>

## Diagram ER

Diagram pokazuje tabele iteracji 4 (sprawa + sprawa_rola wraz z ich słownikami) oraz powiązanie z `dluznik` (iteracja 2). Polimorficzny stos `atrybut` — [Dłużnicy § Diagram ER](dluznicy.md#diagram-er); w iteracji 4 wiersze `att_atd_id = 4` (opisane w sekcji `<code>dbo.atrybut</code>` poniżej) wiążą się ze `sprawa.sp_id` przez polimorficzne `at_ob_id`. Prod-only encje `rachunek_bankowy`, `operator`, `atrybut_sprawa` opisane są w sekcjach `### dbo.<tabela>` poniżej.
