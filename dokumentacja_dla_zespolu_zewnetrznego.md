...
### Iteracja 6 — Wierzytelności, role wierzytelności i ich atrybuty

| Tabela | Zależności FK | Uwagi |
|---|---|---|
| `dbo.wierzytelnosc` | `wi_sp_id → sprawa`, `wi_uko_id → umowa_kontrahent` | Wierzytelności powiązane ze sprawami |
| `dbo.wierzytelnosc_rola` | `wir_wi_id → wierzytelnosc`, `wir_sp_id → sprawa` | Powiązania wierzytelności ze sprawami. Załadować po `wierzytelnosc`. |
...
### Iteracja 7 — Dokumenty i atrybuty dokumentów
...
| Tabela | Zależności FK | Uwagi |
|---|---|---|
| `dbo.ksiegowanie` | `ks_do_id → dokument` (opcjonalny) | Nagłówki księgowań |
...
Iteracja 6  → wierzytelnosc, wierzytelnosc_rola, atrybut (att_atd_id=2)
Iteracja 7  → dokument, atrybut (att_atd_id=1)
...
|---|---|---|---|---|
| STR_01 | BLOCKING | 3 | 101, 102, 103 | Sprawa nie ma przypisanego dłużnika (brak rekordu w `sprawa_rola`) |
...
### 3.4 Integralność strukturalna

Sprawdzają niezmienniki relacyjne nieopisywane przez klucze obce ani ograniczenia NOT NULL — wymagana obecność co najmniej jednego rekordu podrzędnego, bilans podwójnego zapisu, rekordy osierocone oraz anomalie kardynalności względem progów konfigurowalnych.

| Kod | Poziom | Opis |
|---|---|---|
| STR_01 | **BLOKUJĄCE** | Każda sprawa musi mieć co najmniej jeden rekord w `sprawa_rola` — sprawa bez przypisanego dłużnika jest nieprawidłowa |
| STR_02 | INFORMACJA | Sprawa bez powiązanej wierzytelności — dozwolone, ale taka sprawa nie może mieć powiązanych dokumentów ani księgowań |
| STR_03 | **BLOKUJĄCE** | Sprawa bez wierzytelności ma powiązane dokumenty lub dekrety księgowania — niedozwolone, dane muszą zostać poprawione |
| STR_04 | **BLOKUJĄCE** | Księgowanie nie ma żadnego dekretu — zapis księgowy bez linii szczegółowych jest nieprawidłowy |
| STR_05 | **BLOKUJĄCE** | Suma kwot dekretów na jednym księgowaniu nie wynosi zero — naruszenie zasady podwójnego zapisu |
| STR_06 | **BLOKUJĄCE** | Akcja nie ma żadnego rezultatu — każda akcja musi mieć co najmniej jeden rezultat |
| STR_07 | OSTRZEŻENIE | Harmonogram spłat powiązany z nieistniejącą wierzytelnością — rekord osierocony |
| STR_08 | OSTRZEŻENIE | Dłużnik ma nadmierną liczbę numerów telefonu — próg konfigurowalny w `dm_staging.configuration.threshold_config` (klucz: `max_phones_per_dluznik`) |
| STR_09 | OSTRZEŻENIE | Dłużnik ma nadmierną liczbę adresów — próg konfigurowalny w `dm_staging.configuration.threshold_config` (klucz: `max_adresy_per_dluznik`) |
| STR_10 | OSTRZEŻENIE | Sprawa ma nadmierną liczbę akcji — próg konfigurowalny w `dm_staging.configuration.threshold_config` (klucz: `max_akcje_per_sprawa`) |
| STR_11 | OSTRZEŻENIE | Wierzytelność ma nadmierną liczbę dokumentów — próg konfigurowalny w `dm_staging.configuration.threshold_config` (klucz: `max_dokumenty_per_wierzytelnosc`) |

### 3.5 Reguły biznesowe

Sprawdzają zgodność danych z regułami domeny — heurystyki jakości i spójność dat.
...
| Kod | Poziom | Liczba rekordów | Przykładowe ID | Opis |
|---|---|---|---|---|
| STR_06 | BLOCKING | 5 | 201, 202, 203, 204, 205 | Akcja nie ma żadnego rezultatu — wymagany co najmniej jeden |
